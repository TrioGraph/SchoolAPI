...
namespace SchoolAPI.Repositories
{
    public class GendersRepository : IGendersRepository
    {
        ...
        public GendersRepository(SchoolDbContext SchoolDbContext)
        {
        ...
        {
            return await SchoolDbContext.Genders.OrderByDescending(d => d.Id).ToListAsync();
        }

        public async Task<IEnumerable<object>> GetGendersLookup()
        {
            return await SchoolDbContext.Genders
            .Select(s => new
            {
                Id = s.Id,
                Name = s.Name
        ...
        public async Task<Genders> GetGendersById(int id)
        {
            return await SchoolDbContext.Genders.FirstOrDefaultAsync(x => x.Id == id);
        }

        public async Task<Genders> CreateGenders(Genders genders)
        {
            genders.Id = GetNextId();
            await SchoolDbContext.AddRangeAsync(genders);
            await SchoolDbContext.SaveChangesAsync();

            return this.SchoolDbContext.Genders.First(a => a.Id == genders.Id);
        }

        public async Task<Genders> UpdateGenders(int Id, Genders genders)
        {
            var gendersDetails = await SchoolDbContext.Genders.FirstOrDefaultAsync(x => x.Id == Id);
            if (gendersDetails == null)
            {
                return null;
            }
            System.Reflection.PropertyInfo[] propertiesInfo = gendersDetails.GetType().GetProperties();
            foreach(System.Reflection.PropertyInfo propInfo in propertiesInfo)
            {
                propInfo.SetValue(gendersDetails, genders.GetType().GetProperty(propInfo.Name).GetValue(genders));
            }
            gendersDetails.Id = Id;
            await SchoolDbContext.SaveChangesAsync();
            return this.SchoolDbContext.Genders.First(a => a.Id == gendersDetails.Id);
        }

	  public async Task<IEnumerable<Genders>> DeleteGenders(int id)
      {
            var list = await SchoolDbContext.Genders.Where(s => id == s.Id).ToListAsync();
            SchoolDbContext.Genders.RemoveRange(list);
            await SchoolDbContext.SaveChangesAsync();
            return list;
      }

	public async Task<Genders> UpdateGendersStatus(int Id)
        {
            var gendersDetails = await SchoolDbContext.Genders.FirstOrDefaultAsync(x => x.Id == Id);
            if (gendersDetails == null)
            {
      ...
      {
        int? maxId = SchoolDbContext.Genders.Max(p => p.Id);
        if (maxId == null)
        {
            maxId = 0;
        }
...
public Dictionary<string, object> SearchGenders(int userId, string searchString, int pageNumber, int pageSize, string sortColumn, string sortDirection, bool isColumnSearch = false, string columnName = "", string columnDataType = "", string operatorType = "", string value1 = "", string value2 = "")
        {
            ...
            try
            {
                ...
                adapter.SelectCommand = new SqlCommand("GetSearchGenders", new SqlConnection(ConnectionString));
