...
namespace SchoolAPI.Repositories
{
    public class FeeRepository : IFeeRepository
    {
        ...
        public FeeRepository(SchoolDbContext SchoolDbContext)
        {
        ...
        {
            return await SchoolDbContext.Fee.OrderByDescending(d => d.Id).ToListAsync();
        }

        public async Task<IEnumerable<object>> GetFeeLookup()
        {
            return await SchoolDbContext.Fee
            .Select(s => new
            {
                Id = s.Id,
                Name = s.DepartmentId
        ...
        {
            return await SchoolDbContext.Fee.FirstOrDefaultAsync(x => x.Id == id);
        }

        public async Task<Fee> CreateFee(Fee fee)
        {
            fee.Id = GetNextId();
            await SchoolDbContext.AddRangeAsync(fee);
            await SchoolDbContext.SaveChangesAsync();

            return this.SchoolDbContext.Fee.First(a => a.Id == fee.Id);
        }

        public async Task<Fee> UpdateFee(int Id, Fee fee)
        {
            var feeDetails = await SchoolDbContext.Fee.FirstOrDefaultAsync(x => x.Id == Id);
            if (feeDetails == null)
            {
                return null;
            }
            System.Reflection.PropertyInfo[] propertiesInfo = feeDetails.GetType().GetProperties();
            foreach(System.Reflection.PropertyInfo propInfo in propertiesInfo)
            {
                propInfo.SetValue(feeDetails, fee.GetType().GetProperty(propInfo.Name).GetValue(fee));
            }
            feeDetails.Id = Id;
            await SchoolDbContext.SaveChangesAsync();
            return this.SchoolDbContext.Fee.First(a => a.Id == feeDetails.Id);
        }

	  public async Task<IEnumerable<Fee>> DeleteFee(int id)
      {
            var list = await SchoolDbContext.Fee.Where(s => id == s.Id).ToListAsync();
            SchoolDbContext.Fee.RemoveRange(list);
            await SchoolDbContext.SaveChangesAsync();
            return list;
      }

	public async Task<Fee> UpdateFeeStatus(int Id)
        {
            var feeDetails = await SchoolDbContext.Fee.FirstOrDefaultAsync(x => x.Id == Id);
            if (feeDetails == null)
            {
      ...
      {
        int? maxId = SchoolDbContext.Fee.Max(p => p.Id);
        if (maxId == null)
        {
            maxId = 0;
        }
...
public Dictionary<string, object> SearchFee(int userId, string searchString, int pageNumber, int pageSize, string sortColumn, string sortDirection, bool isColumnSearch = false, string columnName = "", string columnDataType = "", string operatorType = "", string value1 = "", string value2 = "")
        {
            ...
            try
            {
                ...
                adapter.SelectCommand = new SqlCommand("GetSearchFee", new SqlConnection(ConnectionString));
