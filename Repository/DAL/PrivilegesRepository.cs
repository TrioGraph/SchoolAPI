...
namespace SchoolAPI.Repositories
{
    public class PrivilegesRepository : IPrivilegesRepository
    {
        ...
        public PrivilegesRepository(SchoolDbContext SchoolDbContext)
        {
        ...
        {
            return await SchoolDbContext.Privileges.OrderByDescending(d => d.Id).ToListAsync();
        }

        public async Task<IEnumerable<object>> GetPrivilegesLookup()
        {
            return await SchoolDbContext.Privileges
            .Select(s => new
            {
                Id = s.Id,
                Name = s.Privilege
        ...
        {
            return await SchoolDbContext.Privileges.FirstOrDefaultAsync(x => x.Id == id);
        }

        public async Task<Privileges> CreatePrivileges(Privileges privileges)
        {
            privileges.Id = GetNextId();
            await SchoolDbContext.AddRangeAsync(privileges);
            await SchoolDbContext.SaveChangesAsync();

            return this.SchoolDbContext.Privileges.First(a => a.Id == privileges.Id);
        }

        public async Task<Privileges> UpdatePrivileges(int Id, Privileges privileges)
        {
            var privilegesDetails = await SchoolDbContext.Privileges.FirstOrDefaultAsync(x => x.Id == Id);
            ...
            await SchoolDbContext.SaveChangesAsync();
            return this.SchoolDbContext.Privileges.First(a => a.Id == privilegesDetails.Id);
        }

	  public async Task<IEnumerable<Privileges>> DeletePrivileges(int id)
      {
            var list = await SchoolDbContext.Privileges.Where(s => id == s.Id).ToListAsync();
            SchoolDbContext.Privileges.RemoveRange(list);
            await SchoolDbContext.SaveChangesAsync();
            return list;
      }
	...
	public async Task<Privileges> UpdatePrivilegesStatus(int Id)
        {
            ...
            privilegesDetails.Active = false;
            await SchoolDbContext.SaveChangesAsync();
            return this.SchoolDbContext.Privileges.First(a => a.Id == privilegesDetails.Id);
        }
      ...
      {
        int? maxId = SchoolDbContext.Privileges.Max(p => p.Id);
        if (maxId == null)
        {
            maxId = 0;
        }
