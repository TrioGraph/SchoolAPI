...
namespace SchoolAPI.Repositories
{
    public class StaffRepository : IStaffRepository
    {
        ...
        public StaffRepository(SchoolDbContext SchoolDbContext)
        {
        ...
        {
            return await SchoolDbContext.Staff.OrderByDescending(d => d.Id).ToListAsync();
        }

        public async Task<IEnumerable<object>> GetStaffLookup()
        {
            return await SchoolDbContext.Staff
            .Select(s => new
            {
                Id = s.Id,
                Name = s.UserId
        ...
        {
            return await SchoolDbContext.Staff.FirstOrDefaultAsync(x => x.Id == id);
        }

        public async Task<Staff> CreateStaff(Staff staff)
        {
            staff.Id = GetNextId();
            await SchoolDbContext.AddRangeAsync(staff);
            await SchoolDbContext.SaveChangesAsync();

            return this.SchoolDbContext.Staff.First(a => a.Id == staff.Id);
        }

        public async Task<Staff> UpdateStaff(int Id, Staff staff)
        {
            var staffDetails = await SchoolDbContext.Staff.FirstOrDefaultAsync(x => x.Id == Id);
            if (staffDetails == null)
            {
                return null;
            }
            System.Reflection.PropertyInfo[] propertiesInfo = staffDetails.GetType().GetProperties();
            foreach(System.Reflection.PropertyInfo propInfo in propertiesInfo)
            {
                propInfo.SetValue(staffDetails, staff.GetType().GetProperty(propInfo.Name).GetValue(staff));
            }
            staffDetails.Id = Id;
            await SchoolDbContext.SaveChangesAsync();
            return this.SchoolDbContext.Staff.First(a => a.Id == staffDetails.Id);
        }

	  public async Task<IEnumerable<Staff>> DeleteStaff(int id)
      {
            var list = await SchoolDbContext.Staff.Where(s => id == s.Id).ToListAsync();
            SchoolDbContext.Staff.RemoveRange(list);
            await SchoolDbContext.SaveChangesAsync();
            return list;
      }

	public async Task<Staff> UpdateStaffStatus(int Id)
        {
            var staffDetails = await SchoolDbContext.Staff.FirstOrDefaultAsync(x => x.Id == Id);
            if (staffDetails == null)
            {
      ...
      {
        int? maxId = SchoolDbContext.Staff.Max(p => p.Id);
        if (maxId == null)
        {
            maxId = 0;
        }
...
public Dictionary<string, object> SearchStaff(int userId, string searchString, int pageNumber, int pageSize, string sortColumn, string sortDirection, bool isColumnSearch = false, string columnName = "", string columnDataType = "", string operatorType = "", string value1 = "", string value2 = "")
        {
            ...
            try
            {
                ...
                adapter.SelectCommand = new SqlCommand("GetSearchStaff", new SqlConnection(ConnectionString));
